      ...
      <div class="row w-100">
              ...
              <div class="accordion w-100" role="tablist"> 
                <b-card no-body class="mb-1 w-100">
                  <b-card-header header-tag="header" class="p-1" role="tab">
                    <b-button @click="openedInitiatives[0] = !openedInitiatives[0]" block v-b-toggle.accordion-1 variant="sucess" class="bg-success text-white">{{ $store.state.language === 'en' ? '' : 'Formation ' }} Leaders<wbr />4<wbr />Development<wbr /> {{ $store.state.language === 'en' ? 'Training' : '' }} <span v-if="openedInitiatives[0]" class="h5"><b-icon icon="caret-up-fill" /></span><span class="h5" v-else><b-icon icon="caret-down-fill" /></span> </b-button>
                  </b-card-header>
                  <b-collapse id="accordion-1" visible accordion="accordion-1" role="tabpanel" class="text-justify">
                    <b-card-body>
                      <b-card-text>{{ $store.state.language === 'en' ? 'OYALD is organising a series of training and capacity development programmes to empower young people for impactful contributions to sustainable development in Africa. ' : 'OJLAD organise une série de programmes de formation et de développement des capacités pour habiliter les jeunes à contribuer de manière significative au développement durable en Afrique. ' }}</b-card-text>
                      <b-card-text>{{ $store.state.language === 'en' ? 'You can join the initiative as a trainee by filling the form below.' : 'Vous pouvez rejoindre cette initiative en tant que stagiaire en remplissant le formulaire ci-dessous.' }}</b-card-text>
                      <hr />
                      <b-form @submit.prevent="submitCapacityBuildingForm">
                        ...
                        <b-form-group
                          ...
                          :key="`field-of-interest-${x}`"
                            >
                            <label for="field-of-interest">{{ $store.state.language === 'en' ? 'Field of interest*' : 'Domaine d\'intérêt' }}</label>
                            <b-form-input
                        ...
                        <b-button-toolbar class="w-100">
                          <b-button-group class="w-100">
                            <b-button size="sm" @click="decrementFieldOfInterestCount" variant="danger" :class="[canRemoveField ? '' : 'disabled']">{{ $store.state.language === 'en' ? 'Remove field' : 'Supprimer un domaine' }}</b-button>
                            <b-button size="sm" @click="incrementFieldOfInterestCount" class="bg-success" :class="[canAddField ? '' : 'disabled']" variant="success">{{ $store.state.language === 'en' ? 'Add another field of interest' : 'Inclure un autre domaine d\'intérêt' }}</b-button>
                          </b-button-group>
                        </b-button-toolbar>
                        ...
                        <b-form-group>
                            <label for="email">{{ $store.state.language === 'en' ? 'Email address*' : 'Adresse e-mail' }}</label>
                            <b-form-input
                            id="email"
                            :placeholder="$store.state.language === 'en' ? 'type here...' : 'Écrire ici'"
                            type="email"
          ...
          <div class="col-12 col-md-6 col-lg-6 col-xl-6">
              <div class="accordion w-100" role="tablist">
                <b-card no-body class="mb-1 w-100">
                  <b-card-header header-tag="header" class="p-1" role="tab">
                    <b-button @click="openedInitiatives[1] = !openedInitiatives[1]" block v-b-toggle.accordion-2 variant="sucess" class="bg-success text-white">{{ $store.state.language === 'en' ? 'Do you know Africa?' : 'Connaissez-vous l\'Afrique?' }} <span v-if="openedInitiatives[1]" class="h5"><b-icon icon="caret-up-fill" /></span><span class="h5" v-else><b-icon icon="caret-down-fill" /></span> </b-button>
                  </b-card-header>
                  <b-collapse id="accordion-2" visible accordion="accordion-2" role="tabpanel">
                    <b-card-body>
                      <center class="p-l-r">
                        <p class="text-justify">{{ $store.state.language === 'en' ? 'In this initiative, OYALD provides regular historical information and current events updates about Africa on our social media channels. This serves as a readily available source of relevant knowledge about our African heritage, and helps to keep our social media followers up-to-date on the happenings in the continent.' : 'Dans cette initiative, OJLAD fournit régulièrement des informations historiques et des mises à jour sur les événements actuels en Afrique sur nos canaux de médias sociaux. Cela sert de source facilement accessible de connaissances pertinentes sur notre héritage africain, et aide à garder nos adeptes des médias sociaux au courant des événements sur le continent.' }}</p>
                        <p class="text-justify">{{ $store.state.language === 'en' ? 'Kindly follow us on, and subscribe to, our social media pages/profiles to get these information updates about Africa.' : 'Veuillez nous suivre sur, et vous abonner à, nos pages/profils de médias sociaux pour obtenir ces mises à jour sur l\'Afrique.' }}</p>
                      </center>
                      <br />
