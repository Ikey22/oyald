<template>
  <div class="w-100 container-fluid">
    <h1
      class="text-center font-weight-bold w-100 text-success"
    ...
    <p class="text-center w-100 font-weight-bold">
      {{
        $store.state.language == "en"
    ...
    <p class="w-100 text-center">
      <span class="p-1 bg-danger text-white font-weight-bold rounded">{{
        $store.state.language == "en" ? "Note:" : "Noter"
      }}</span>
      {{
        $store.state.language == "en"
          ? "Fields labelled with asterisk (*) are mandatory, the rest are optional"
          : "Les champs marqués d'un astérisque (*) sont obligatoires, les autres sont facultatifs"
      }}
    </p>
    <center>
      <b-form
      ...
      >
        <b-form-row class="row w-100">
          ...
          <b-form-group class="col-12 col-sm-12 col-md-6 col-lg-6 col-xl-6">
            <label for="surname">{{
              $store.state.language == "en" ? "Surname*" : "Nom de famille*"
            }}</label>
            <b-form-input
              id="surname"
        ...
        <b-form-row class="row w-100">
          <b-form-group class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <label for="subject">{{
              $store.state.language == "en" ? "Subject*" : "Matière*"
            }}</label>
            <b-form-input
              id="subject"
              v-model="subject"
        ...
        <b-button type="submit" class="bg-green w-100">{{
